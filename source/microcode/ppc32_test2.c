...
int syscall_handler(unsigned int syscall, unsigned int a1, unsigned int a2, unsigned int a3, unsigned int pc)
{
  ...
  case 10: { // version string
    static const char* VERSION_STRING = "\n\n(ppc32_test2.c) ROMMON Emulation Microcode\n"; 
    return (unsigned int)VERSION_STRING;
  }

  case 8: { // restart IOS
    remote_print(CONSOLE, "\n\n(ppc32_test2.c) ROM: reload requested...\n");
    if (remote_ctrl.restart_ios) {
      remote_print(CONSOLE, "(ppc32_test2.c) ROM: Restarting IOS...\n");
...
{
  // init
  remote_print(CONSOLE, "(ppc32_test2.c) ROMMON emulation microcode.\n\n");
  remote_print(LOG, "(ppc32_test2.c) Microcode has started.\n");
  ...
  // run
  do {
    remote_print(CONSOLE, "(ppc32_test2.c) Launching IOS image at 0x%x...\n", remote_ctrl.elf_entry_point);
    launch_image(remote_ctrl.elf_entry_point);
  } while(remote_ctrl.restart_ios);

  // terminate
  remote_print(CONSOLE, "(ppc32_test2.c) Image returned to ROM.\n");
  remote_ctrl.stop_virtual_machine = 1;
}
