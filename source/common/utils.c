...
void *m_memalign(size_t boundary,size_t size)
{
   ...
   if (!(p= __mingw_aligned_malloc(size,boundary)))
#else
...
int memzone_create_file(char *filename,size_t len,u_char **ptr)
{
   int fd,mode;
   #ifndef __MINGW32__
   mode = O_CREAT|O_RDWR,S_IRWXU;
   #else
   mode = O_CREAT|O_RDWR|O_BINARY,S_IRWXU;
   #endif
...
int memzone_open_cow_file(char *filename,size_t len,u_char **ptr)
{
   int fd,mode;
   #ifndef __MINGW32__
   mode = O_RDONLY,S_IRWXU;
   #else
   mode = O_RDONLY|O_BINARY,S_IRWXU;
   #endif
