...
/* generic code to initialize Winsock 2.0 */
...
/*
According to all the WinPcap documents, you should always have a dynamic set
never use static linking.....

Plus this will let you run on machines with no pcap.
*/

void loadpcap()
{
		 hPcapLib = LoadLibraryA(Pcaplib_name);
			if(hPcapLib==0)
		{
			printf("Failed to load %s\n",Pcaplib_name);
			printf("Its HIGHLY recommended you have a working WinPcap!\n\n");
			return;
		}
		_pcap_lib_version =(PCAP_LIB_VERSION)GetProcAddress(hPcapLib,"pcap_lib_version");
		_pcap_open_live=(PCAP_OPEN_LIVE)GetProcAddress(hPcapLib,"pcap_open_live");		
		_pcap_sendpacket=(PCAP_SENDPACKET)GetProcAddress(hPcapLib,"pcap_sendpacket");		
		_pcap_setnonblock=(PCAP_SETNONBLOCK)GetProcAddress(hPcapLib,"pcap_setnonblock");	
		_pcap_next=(PCAP_NEXT)GetProcAddress(hPcapLib,"pcap_next");		
		_pcap_close=(PCAP_CLOSE)GetProcAddress(hPcapLib,"pcap_close");
		_pcap_getnonblock=(PCAP_GETNONBLOCK)GetProcAddress(hPcapLib,"pcap_getnonblock");
		_pcap_compile=(PCAP_COMPILE)GetProcAddress(hPcapLib,"pcap_compile");
		_pcap_setfilter=(PCAP_SETFILTER)GetProcAddress(hPcapLib,"pcap_setfilter");
		_pcap_datalink_name_to_val=(PCAP_DATALINK_NAME_TO_VAL)GetProcAddress(hPcapLib,"pcap_datalink_name_to_val");
		_pcap_open_dead=(PCAP_OPEN_DEAD)GetProcAddress(hPcapLib,"pcap_open_dead");
		_pcap_snapshot=(PCAP_SNAPSHOT)GetProcAddress(hPcapLib,"pcap_snapshot");
		_pcap_dump=(PCAP_DUMP)GetProcAddress(hPcapLib,"pcap_dump");
		_pcap_dump_open=(PCAP_DUMP_OPEN)GetProcAddress(hPcapLib,"pcap_dump_open");
		_pcap_dump_close=(PCAP_DUMP_CLOSE)GetProcAddress(hPcapLib,"pcap_dump_close");
		_pcap_dump_flush=(PCAP_DUMP_FLUSH)GetProcAddress(hPcapLib,"pcap_dump_flush");
		_pcap_setdirection=(PCAP_SETDIRECTION)GetProcAddress(hPcapLib,"pcap_setdirection");
		_pcap_findalldevs=(PCAP_FINDALLDEVS)GetProcAddress(hPcapLib,"pcap_findalldevs");
		_pcap_freealldevs=(PCAP_FREEALLDEVS)GetProcAddress(hPcapLib,"pcap_freealldevs");
...
#endif //End of Win32 startup code.
...
int main(int argc,char *argv[])
{
   ...
   #ifdef __MINGW32__
   #include "..\contrib\drmingw-0.8.0-win32\include\exchndl.h"
   //ExcHndlInit();
   startwinsock();
   loadpcap();
   #endif
