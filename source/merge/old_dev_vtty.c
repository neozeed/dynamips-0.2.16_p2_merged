...
static int vtty_tcp_conn_wait(vtty_t *vtty)
{
    ...
    for (res=res0;(res && (nsock < VTTY_MAX_FD));res=res->ai_next)
    {
            ...
            (listen(vtty->fd_array[nsock],1) < 0))
        {
            ...
            closesocket(vtty->fd_array[nsock]);
...
#endif	//no serial stuff in MinGW
