                          ...
                          char *format,...)
{
   va_list ap;
   size_t n = 0;
   char buffer[8192];   //really no idea how big..
   ...
   if (conn != NULL) {
      ...
      fflush(conn->out);
#else
      memset(buffer,0x0,8192);
      sprintf(buffer,"%3d%s",code,(done)?"-":" ");
...
{   
   ...
   u_long iMode=0;	//Set to a blocking socket.
   ioctlsocket(conn->in,FIONBIO,&iMode);

   while(conn->active) {
      //if (!fgets(buffer,sizeof(buffer),conn->in))
      //   break;
      //MinGW can't read a socket like a stdio file
      //So here is my lame DIY ring buffer.
        char rbuffer[2];
        int rc;
        ...
        rc=1;
        while( (rbuffer[0]!=0xa) && (rc>0) )
                {
                rc=recv(conn->client_fd,&rbuffer,1,0);
                if(rc>0)
                        sprintf(buffer+strlen(buffer),"%s",rbuffer);
                else
                        conn->active=FALSE;
                }
//end MinGW mod
...
static void hypervisor_close_conn(hypervisor_conn_t *conn)
{
   if (conn != NULL) {
      ...
      free(conn);
//evil!
//this closes dynamips once ANY client disconnects
//MinGW
   hypervisor_running = FALSE;
   }
}
...
//So on Unix we set these as file streams
//Not going to fly on Win32 / MinGW
//
#if 0
   ...
   setlinebuf(conn->out);
#else
	//These never worked... do we even need to?
        //conn->in=client_fd;
        //conn->out=client_fd;
#endif
...
static int hypervisor_close_conn_list_count=0;
...
//      hypervisor_close_conn_list_count++;
//      if(hypervisor_close_conn_list_count>3) {
         hypervisor_close_conn_list(TRUE);		//this is now eating CPU like crazy
//         hypervisor_close_conn_list_count=0;
//         printf("f");
//         }
